...
class RunBetData:
    ...
    def get_init_price(self):
        data_json = self._get_json_data()
        return data_json["runBet"]["init_price"]
    ...
    def get_quantity(self, exchange_method=True):
        ...
        data_json = self._get_json_data()
        # 买入与卖出操作对应的仓位不同
        cur_step = data_json["runBet"]["step"] if exchange_method else data_json["runBet"]["step"] - 1
    ...
    def modify_price(self, deal_price, step):
        print("开始修改补仓价和网格价")
        # 初始价固定方案
        deal_price = self.get_init_price()
        data_json = self._get_json_data()
        data_json["runBet"]["next_buy_price"] = round(
            deal_price * (1 - data_json["config"]["double_throw_ratio"] / 100), 2)  # 保留2位小数
        data_json["runBet"]["grid_sell_price"] = round(
            deal_price * (1 + data_json["config"]["profit_ratio"] / 100), 2)
